...
const main = async () => {
    ...
    await borrowDAI(wethAddress, lendingPool, amountDaiToBorrowWei, deployer)
}

async function borrowDAI(wethAddress, lendingPool, amountDaiToBorrowWei, account) {
    const borrowTx = await lendingPool.borrow(wethAddress, amountDaiToBorrow, 1, 0, deployer)
    await borrowTx.wait()
